...
class Anime():
    ...
    def __request(self, req, no_cookies=False, show_fail=True, max_retry=3, addition_header=None):
        ...
        if not self._cookies:
            ...
            if 'set-cookie' in f.headers.keys():  # 发现server响应了set-cookie
                ...
                elif 'cf_use_ob' in f.headers.get('set-cookie') and 'BAHARUNE' not in f.headers.get('set-cookie'):
                    # 20211222 預防網絡不穩造成更新出錯
                    err_print(self._sn, '網絡不穩定暫停cookie更新一次', display=False)
    ...
    def __segment_download_mode(self, resolution=''):
        ...
        key_uri = key_uri.split('URI="')[1].split('",IV=')[0]  # 把 key 的链接提取出来
